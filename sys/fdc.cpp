...
// Добавьте в конец файла fdc.cpp, перед закрывающей скобкой

// Функция с повторами при ошибке
int fdc_read_sector_with_retry(uint8_t drive, uint8_t head, uint8_t track, uint8_t sector, uint8_t* buffer) {
    ...
    for(int retry = 0; retry < max_retries; retry++) {
        ...
        // Проверяем успешность чтения (простейшая проверка)
        // Если первый байт не 0xFF - считаем что прочитали
