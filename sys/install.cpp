...
static void add_install_log(const char* stage, const char* message, int success) {
    if(install_log_count < 200) {
        ...
        *p = '\0';
        ...
        // Копируем stage и message вручную
...
static void create_swap_partition(int size_mb) {
    ...
    // Находим свободное место после установки
    uint32_t total_used_sectors = 0;
    
    // Сканируем использованные сектора
    for(int i = 0; i < 32; i++) {
        uint16_t dir_buf[256];
        read_sector(100 + i, dir_buf);
        // Подсчёт использованных секторов...
    }
    
    // Swap начинается с сектора 50000 (после основной ФС)
...
// Простая функция получения клавиши
...
// Копирование файла с CD-ROM на HDD
...
// Типы файловых систем CD-ROM
#define CD_FS_ISO9660    1  // Стандартный ISO 9660
#define CD_FS_JOLIET     2  // Microsoft Joliet (unicode)
#define CD_FS_ROCKRIDGE  3  // Rock Ridge (Unix permissions)
#define CD_FS_ELTORITO   4  // El Torito (bootable)
#define CD_FS_UDF        5  // UDF (DVD/Blu-ray)
#define CD_FS_HFS        6  // Apple HFS
#define CD_FS_HFS_PLUS   7  // Apple HFS+
#define CD_FS_HYBRID     8  // Гибридный (ISO+UDF+HFS)
#define CD_FS_RAW        9  // Сырой образ
...
// Проверка ISO 9660 (PVD в секторе 16)
static int detect_iso9660(void) {
    ...
    for(int offset = 0; offset < 2048 - 100; offset++) {
        if(sector[offset] == 1 &&  // Primary Volume Descriptor
           sector[offset+1] == 'C' &&
           sector[offset+2] == 'D' &&
           sector[offset+3] == '0' &&
           sector[offset+4] == '0' &&
           sector[offset+5] == '1') {
            ...
            cd_info.root_lba = *(uint32_t*)(sector + offset + 158);  // Root dir LBA
            cd_info.root_size = *(uint32_t*)(sector + offset + 166); // Root dir size
            cd_info.total_sectors = *(uint32_t*)(sector + offset + 80); // Volume size
...
// Проверка Joliet (Supplementary Volume Descriptor)
static int detect_joliet(void) {
    // Joliet использует Unicode в SVD (обычно сектор 17 или 18)
    for(int lba = 17; lba < 25; lba++) {
        uint8_t sector[2048];
        if(atapi_read_sector(lba, sector) <= 0) continue;
        
        for(int offset = 0; offset < 2048 - 100; offset++) {
            if(sector[offset] == 2 &&  // Supplementary Volume Descriptor
               sector[offset+1] == 'C' &&
               sector[offset+2] == 'D' &&
               sector[offset+3] == '0' &&
               sector[offset+4] == '0' &&
...
// Проверка Rock Ridge (Unix расширения)
static int detect_rockridge(void) {
    // Rock Ridge использует PVD с расширенными атрибутами
    uint8_t sector[2048];
    if(atapi_read_sector(16, sector) <= 0) return 0;
    ...
    for(int offset = 0; offset < 2048 - 4; offset++) {
        if(sector[offset] == 'R' && sector[offset+1] == 'R' &&
           sector[offset+2] == 0x49 && sector[offset+3] == 0x50) {
            // Нашли Rock Ridge Interchange Protocol
...
// Проверка UDF (Universal Disk Format)
static int detect_udf(void) {
    // UDF Anchor Volume Descriptor Pointer в секторе 256
    uint8_t sector[2048];
    ...
    // Пробуем сектор 512 (второй якорь)
...
static int detect_hfs(void) {
    ...
    // HFS Master Directory Block в секторе 2
    if(atapi_read_sector(2, sector) <= 0) return 0;
    
    if(sector[0] == 0x42 && sector[1] == 0x44) {  // "BD" = HFS
...
static int scan_cdrom_fs(void) {
    ...
    // Пробуем прочитать первый сектор
    uint8_t test[2048];
    ...
    // 1. Проверяем UDF (DVD/Blu-ray)
    if(detect_udf()) {
        kprint_color("[SCAN] UDF filesystem found\n", TXT_GREEN);
        found |= CD_FS_UDF;
    }
    
    // 2. Проверяем HFS (Apple)
    if(detect_hfs()) {
    ...
    if(cd_info.total_sectors > 0) {
        kprint("[SCAN] Size: ");
        kprint_int(cd_info.total_sectors / 2);  // Примерно в KB
        kprint(" KB\n");
    }
...
// ==================== КОПИРОВАНИЕ С CD-ROM ====================

// Чтение директории ISO 9660
static void read_iso_directory(uint32_t dir_lba, uint32_t dir_size, 
    ...
    uint8_t* dir_buffer = (uint8_t*)0x200000;  // 2 MB buffer for directory
...
static int copy_cd_directory(uint32_t dir_lba, uint32_t dir_size, 
    ...
    while(offset < dir_size) {
        ...
        if(name_len > 0) {
            ...
            // Создаём имя для файловой системы (обрезаем до 11 символов)
            char short_name[13] = {0};
            int nc = 0;
            for(int j = 0; j < fn_len && nc < 11; j++) {
                if(filename[j] >= 'a' && filename[j] <= 'z') {
                    short_name[nc++] = filename[j] - 32;  // В верхний регистр
            ...
            if(nc == 0) {
                // Если имя не удалось преобразовать, используем generic
                static int gen_count = 0;
                my_sprintf(short_name, "CDROM_%04d", gen_count++);
            }
            
            if(flags & 0x02) {  // Директория
                // Создаём директорию
...
static void quick_install_from_cd(void) {
    ...
    // Создаём структуру директорий на HDD
    uint16_t root_sector = 100;
    
    // Копируем всё дерево файлов
...
create_file(root_sector, "README.txt", readme_text);
    ...
    // Реально перемещаем файлы для дефрагментации
kprint("Defragmenting filesystem...\n");

uint16_t dir_buf[256];
read_sector(100, dir_buf);

// Сначала сканируем все используемые сектора
...
// Находим фрагментированные файлы
int fragmented = 0;

for(int pass = 0; pass < 2; pass++) {  // 2 прохода для надёжности
    kprint("\nDefrag pass ");
    kprint_int(pass + 1);
    kprint("/2...\n");
    ...
    for(int i = 0; i < 32; i++) {
        ...
        if(!is_dir && file_sector > 0 && file_size > 0) {
            // Проверяем, не фрагментирован ли файл
            ...
            if(file_sector != expected_sector || pass == 0) {
                ...
                if(has_data || file_size > 0) {
                    // Сохраняем резервную копию во временный сектор
                    ...
                    if(write_ok) {
                        // Обновляем указатель в директории
                        dir_buf[i*8 + 6] = expected_sector;
                        write_sector(100, dir_buf);
                        
                        // Очищаем старый сектор (опционально)
                        uint16_t zero_buf[256];
...
static void inst_draw_menu(void) {
    ...
    // Опция 4: Установка с CD-ROM (новая!)
    if(selected == 4) {
...
static void stage1_install(void) {
    ...
    while(1) {
        ...
        else if((key == 0xE1 || key == 's') && selected < 5) {  // Изменили с 4 на 5
            selected++;
        }
        else if(key == '\n') {
            if(selected == 0) {
