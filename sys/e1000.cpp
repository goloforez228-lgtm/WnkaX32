...
int e1000_send(netif_t* netif, uint8_t* data, uint32_t len) {
    ...
    uint16_t tdt = e1000[E1000_TDT / 4];
    uint16_t next = tdt;
    ...
    e1000[E1000_TDT / 4] = (next + 1) % 256;
    
    while(!(tx_desc[next].status & 0xFF));
...
int e1000_recv(netif_t* netif, uint8_t* buffer, uint32_t max_len) {
    ...
    uint16_t rdh = e1000[E1000_RDH / 4];
    uint16_t rdt = e1000[E1000_RDT / 4];
    
    if(rdh == rdt) return 0;
    
    if(!(rx_desc[rdh].status & 1)) return 0;
    ...
    e1000[E1000_RDT / 4] = (rdh + 1) % 256;
    
    return len;
}

int e1000_link_up(void) {
    return (e1000[E1000_STATUS / 4] & (1 << 1)) ? 1 : 0;
}

void e1000_dump_stats(void) {
    kprint("\n=== E1000 STATISTICS ===\n");
    kprint("Status: 0x"); kprint_hex32(e1000[E1000_STATUS / 4]); kprint("\n");
    kprint("Link: ");
    if(e1000_link_up()) kprint_color("UP\n", TXT_GREEN);
    else kprint_color("DOWN\n", TXT_RED);
    kprint("RX Descriptors: "); kprint_int(e1000[E1000_RDH / 4]); kprint("/");
    kprint_int(e1000[E1000_RDT / 4]); kprint("\n");
    kprint("TX Descriptors: "); kprint_int(e1000[E1000_TDH / 4]); kprint("/");
    kprint_int(e1000[E1000_TDT / 4]); kprint("\n");
}
