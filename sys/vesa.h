...
static void vesa_enable(void) {
    ...
    if(vbe_id < 0xB0C0 || vbe_id > 0xB0C5) {
        ...
        if(!vesa_detect_bga()) {
            ...
            kprint_color("  Your hardware doesn't support\n", TXT_RED);
            kprint_color("  this video mode.\n", TXT_RED);
            kprint_color("========================================\n\n", TXT_RED);
            kprint_color("Possible solutions:\n", TXT_YELLOW);
            kprint_color("  1. Use a real PC with VBE 2.0+\n", TXT_WHITE);
            kprint_color("  2. Use QEMU: qemu -vga std\n", TXT_WHITE);
            kprint_color("  3. Use VirtualBox with VBoxSVGA\n", TXT_WHITE);
            kprint_color("  4. Use VMware with SVGA\n", TXT_WHITE);
    ...
    for(int r = 0; r < num_res; r++) {
        int try_w = resolutions[r][0];
        int try_h = resolutions[r][1];
        
        vesa_outw(VBE_DISPI_IOPORT_INDEX, VBE_DISPI_INDEX_XRES);
        vesa_outw(VBE_DISPI_IOPORT_DATA, try_w);
        vesa_iodelay();
        
        vesa_outw(VBE_DISPI_IOPORT_INDEX, VBE_DISPI_INDEX_YRES);
        vesa_outw(VBE_DISPI_IOPORT_DATA, try_h);
        vesa_iodelay();
        
        vesa_outw(VBE_DISPI_IOPORT_INDEX, VBE_DISPI_INDEX_BPP);
        vesa_outw(VBE_DISPI_IOPORT_DATA, 16);
        vesa_iodelay();
        ...
        if(vesa_probe_lfb()) {
            kprint_color("[VESA] Resolution: ", TXT_GREEN);
            kprint_int(try_w); kprint("x"); kprint_int(try_h); kprint("\n");
            vesa_w = try_w;
            vesa_h = try_h;
            vesa_pitch = try_w * 2;
            found_res = 1;
    ...
    if(!found_res) {
        kprint_color("[VESA] Could not set any resolution!\n", TXT_RED);
        kprint_color("THIS VIDEO CARD IS NOT SUPPORTED\n", TXT_RED);
        vesa_ok = 0;
        return;
    }
