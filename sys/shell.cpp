...
extern "C" void process_command(char* input_buffer, int& input_ptr) {
    ...
    static int system_integrity_ok = 1;
    
    if(!integrity_checked) {
        integrity_checked = 1;
        
        const char* required_bin_files[] = {
            "ls", "cat", "mkdir", "rm", "cp", "mv", "echo", "clear", 
            "help", "reboot", "shutdown", "fetch", "df", "wnkc", "time"
        };
        int required_count = 15;
        int missing_count = 0;
        ...
        for(int i = 0; i < 32; i++) {
            ...
            if(my_strcmp("bin", name) == 0 && ((char*)root_buf)[i*16 + 11] == 1) {
        ...
        if(bin_sector != 0) {
            ...
            for(int i = 0; i < required_count; i++) {
                ...
                for(int j = 0; j < 32; j++) {
                    char name[12] = {0};
                    for(int k = 0; k < 11; k++) name[k] = ((char*)bin_buf)[j*16 + k];
                    if(my_strcmp(required_bin_files[i], name) == 0) {
                        found = 1;
                        break;
                    }
                }
        ...
        if(missing_count > 3 || bin_sector == 0) {
            system_integrity_ok = 0;
            
            clear_screen_bg(COLOR_RED);
            
            draw_shadow_window(10, 4, 60, 14, COLOR_RED, TXT_WHITE, "SYSTEM INTEGRITY FAILURE");
            
            draw_dframe(12, 6, 56, 10, COLOR_BLACK, TXT_YELLOW);
            
            kprint_at("CRITICAL SYSTEM FILES ARE MISSING!", 16, 8, (COLOR_BLACK << 4) | TXT_RED);
            
            char missing_str[64];
            my_sprintf(missing_str, "Missing files in /bin: %d of %d", missing_count, required_count);
            kprint_at(missing_str, 18, 10, (COLOR_BLACK << 4) | TXT_YELLOW);
            
            kprint_at("The system is damaged and UNSTABLE!", 18, 12, (COLOR_BLACK << 4) | TXT_RED);
            
            kprint_at("", 0, 13, 0);
            kprint_color("Options:\n", TXT_CYAN);
            kprint_color("  [R] - Reinstall system\n", TXT_YELLOW);
            kprint_color("  [Any key] - Continue (NOT RECOMMENDED)\n", TXT_RED);
            
            draw_hline(12, 19, 56, COLOR_BLACK, TXT_WHITE, D_HLINE);
            ...
            if(key == 0x13) {
                ...
                kprint_color("\nContinuing with damaged system! Stability NOT guaranteed!\n", TXT_YELLOW);
                inst_delay(2000);
                clear_screen();
            }
        } else {
            kprint_color("System integrity check passed\n", TXT_GREEN);
        }
    }
    
    static int warning_shown = 0;
    if(!system_integrity_ok && !warning_shown) {
        warning_shown = 1;
        
        draw_frame(10, 20, 60, 4, COLOR_RED, TXT_WHITE);
        kprint_at("WARNING: System is damaged! Commands may fail.", 14, 21, (COLOR_RED << 4) | TXT_YELLOW);
        kprint_at("Please reinstall as soon as possible.", 18, 22, (COLOR_RED << 4) | TXT_WHITE);
    }
...
else if(my_strcmp(cmd_copy, "ramfs") == 0) {
    if(arg[0] == '\0') {
        kprint("RAMFS commands:\n");
        kprint("  ramfs init   - initialize RAMFS\n");
        kprint("  ramfs mount  - mount RAMFS\n");
        kprint("  ramfs ls     - list files\n");
        kprint("  ramfs mkdir  - create directory\n");
        kprint("  ramfs info   - show info\n");
        kprint("  ramfs find   - find file\n");
    }
}
    else if(my_strcmp(arg, "init") == 0) {
        ramfs_init(16);
    }
    else if(my_strcmp(arg, "mount") == 0) {
        ramfs_mount();
    }
    else if(my_strcmp(arg, "info") == 0) {
        ramfs_info();
    }
else if(my_strncmp(arg, "ls", 2) == 0) {
    char* path = arg + 2;
    while(*path == ' ') path++;
    if(*path == '\0') {
        ramfs_ls("/");
...
else if(my_strncmp(arg, "mkdir", 5) == 0) {
    char* path = arg + 5;
    while(*path == ' ') path++;
    if(ramfs_mkdir(path) == 0) {
        kprint("Directory created\n");
    } else {
        kprint("Failed to create directory\n");
    }
}
else if(my_strncmp(arg, "find", 4) == 0) {
    char* path = arg + 4;
    while(*path == ' ') path++;
    ramfs_find(path);
}
