...
// ==================== КОРОТКИЕ КОМАНДЫ ДЛЯ DEBUG MODE ====================

else if(my_strcmp(cmd_copy, "rs") == 0) {  // read sector
    uint32_t sector = 0;
    char* p = arg;
    while(*p >= '0' && *p <= '9') {
...
else if(my_strcmp(cmd_copy, "ws") == 0) {  // write sector (zero)
    uint32_t sector = 0;
    char* p = arg;
    while(*p >= '0' && *p <= '9') {
        sector = sector * 10 + (*p - '0');
...
else if(my_strcmp(cmd_copy, "md") == 0) {  // make directory
    char* dirname = arg;
...
else if(my_strcmp(cmd_copy, "rd") == 0) {  // remove directory
    char* dirname = arg;
    while(*dirname == ' ') dirname++;
...
else if(my_strcmp(cmd_copy, "touch") == 0) {  // create file
    char* filename = arg;
...
else if(my_strcmp(cmd_copy, "w") == 0) {  // write to file
    char* rest = arg;
    char filename[32] = {0};
    char* content = NULL;
...
else if(my_strcmp(cmd_copy, "r") == 0) {  // read file
    char* filename = arg;
    while(*filename == ' ') filename++;
...
// ==================== ПОЛЬЗОВАТЕЛИ ====================
    ...
    // Создаём запись в /etc/passwd (сектор 107)
    ...
    // Сохраняем пароль в сектор 108 для пользователя
    ...
    // Текущий пользователь (пока debug)
    kprint("debug\n");
}
...
// В shell.cpp добавьте:
    ...
    // Поддержка аргументов: runelf <sector> <filename>
...
// ==================== BROWSE КОМАНДА ====================
    ...
    if(url[0] == '\0') {
        ...
        // Добавляем http:// если нет префикса
        char full_url[256];
        if(url[0] != 'h') {
...
// ==================== NETINFO КОМАНДА ====================
    ...
    kprint("IP: ");
    kprint_int(e1000_netif.ip.addr[0]); kprint(".");
    kprint_int(e1000_netif.ip.addr[1]); kprint(".");
    kprint_int(e1000_netif.ip.addr[2]); kprint(".");
    kprint_int(e1000_netif.ip.addr[3]);
    kprint("\n");
    
    kprint("Gateway: ");
    kprint_int(e1000_netif.gateway.addr[0]); kprint(".");
    kprint_int(e1000_netif.gateway.addr[1]); kprint(".");
    kprint_int(e1000_netif.gateway.addr[2]); kprint(".");
    kprint_int(e1000_netif.gateway.addr[3]);
    kprint("\n");
