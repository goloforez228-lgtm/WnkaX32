...
// Простая функция контрольной суммы (CRC32 упрощённый)
...
// Загрузка WNX файла из файловой системы
...
int wnx_execute(wnx_context_t* ctx) {
    ...
    // Передаём аргументы через регистры
...
int wnx_list_files(void) {
    ...
    for(int i = 0; i < 32; i++) {
        ...
        if(name[0] != 0 && name[0] != 0xE5) {
            // Проверяем, является ли файл WNX
            uint16_t hdr_buf[256];
            ...
            if(magic == WNX_MAGIC) {
                ...
                // Читаем имя программы из заголовка
